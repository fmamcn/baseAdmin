...
class AdminBase extends Base
{
    ...
    public function getNavbar()
    {
        ...
        if (session('admin_auth.username') != config('administrator')) {
            $access      = model('authGroupAccess')->with('authGroup')->where('uid', session('admin_auth.admin_id'))->find();
            $where['id'] = ['in', $access['rules']];
        }
        return collection(model('authRule')->where($where)->order('sort_order asc')->select())->toArray();
    }
}
